...
The method is deliberately *quick*: it is a fast, deterministic
estimate, not a full Bayesian fit. The approximations that buy that
speed are listed honestly at the end.
...
- $`K_{\text{space}}`$ and $`K_{\text{time}}`$ are **kernels** — they
  set how correlated two cells are as a function of how far apart they
  are.
...
![](fitting-the-model_files/figure-html/unnamed-chunk-3-1.png)
...
![](fitting-the-model_files/figure-html/unnamed-chunk-5-1.png)
...
instead of signal. Without it, the kernel would have to explain the
noisy plug-in field with smoothness alone, badly distorting the length
scales.
...
one for time — which is astronomically cheaper, and gives the *exact*
same answer. This is the whole reason the estimate takes a second rather
than hours.
...
the **true** ones (black dashed). If the method worked, they overlap.
...
![](fitting-the-model_files/figure-html/unnamed-chunk-8-1.png)
...
![](fitting-the-model_files/figure-html/unnamed-chunk-8-2.png)
