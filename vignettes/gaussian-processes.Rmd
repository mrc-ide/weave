...
ggplot(curves, aes(lag, correlation, colour = length_scale)) +
  ...
  scale_colour_manual(values = unname(weave_pal[c("rose", "teal")]), name = NULL) +
...
measurement scatter rather than smooth signal. In `weave` it is estimated
alongside the kernel parameters as the *nugget ratio* — noise variance as a
share of signal variance — and the [walkthrough](walkthrough.html) explains
why the ratio, rather than the absolute noise, is the natural quantity.
:::
...
ggplot() +
  ...
  labs(x = "Day", y = "Hummingbird visits",
       ...
       subtitle = "grey = observations · teal = mean + 95% band · deep green dashed = truth") +
  theme_weave()
```
