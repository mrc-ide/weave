...
variance that is observation noise rather than smooth signal. Its role — and
why it is a *ratio* — is explained in part 2, where it enters the model. (For
what each kernel looks like and how its parameter changes the curves it
generates, see the [*gentle introduction*](gaussian-processes.html).)
...
```{r missingness-map, echo = FALSE, fig.height = 3.8, fig.alt = "Heatmap of sites by weeks: observed weeks in pale sage, missing weeks in rose, with the missing weeks forming horizontal clustered runs within sites"}
miss_map <- data.frame(
...
ggplot(miss_map, aes(t, id, fill = status)) +
  ...
  scale_fill_manual(values = c(observed = weave_grid,
                               missing = weave_cols[["held_out"]]),
                    name = NULL) +
  scale_y_continuous(breaks = seq(5, n, by = 5), expand = c(0, 0)) +
  scale_x_continuous(expand = c(0, 0)) +
  labs(x = "Week", y = "Site",
       title = "Where the gaps are",
       subtitle = "missing weeks (rose) come in clustered runs, like real reporting dropouts") +
  theme_weave()
```

Here are the first four sites. The **deep-green line** is the true underlying
mean (\( \lambda = e^{\mu_s + f_{st}} \)); **grey points** are the counts we
actually observe; and **rose points** are the *held-out truth* at the weeks
...
ggplot() +
  ...
  labs(x = "Week", y = "Cases",
       ...
       subtitle = "deep green = true mean · grey = observed · rose = held-out truth") +
  theme_weave()
```
...
agree: a **stand-in for the latent field** we cannot see (starting from a
cheap plug-in, with the model itself progressively refilling the missing
weeks), and an **exact score** — the marginal likelihood — maximised over
the hyperparameters given the current stand-in. With no missing data the
loop collapses to a single pass; with gaps it is exactly the EM algorithm.
...
divide by its standard deviation. Weeks with no data enter as the per-site
mean — zero after centring — a deliberately neutral starting fill that the
EM loop below revisits.
:::
...
On the observed weeks, the plug-in field tracks the true latent field with
observation noise layered on top. On the *missing* weeks it is flat and
zero — every gap looks like a stretch where the signal died, and left alone
that biases the length-scales downward (the seasonal and long-run scales
suffer most). Setting `refine = TRUE` treats the plug-in field as just the
starting point of an EM loop:

::: {.intuition}
This loop is exactly the **EM algorithm** for a fit with missing data,
alternating two steps (the plug-in field with its neutral fill is iteration
zero):
...
The plug-in start is flat at zero in every gap; the first EM pass replaces
those stretches with the GP conditional mean; the second pass barely moves
it — the loop has converged. The observed weeks never change, only the gaps:

```{r em-rounds, echo = FALSE, fig.height = 4.2, fig.alt = "One site's latent field across weeks: shaded bands mark missing stretches; the plug-in line sits flat at zero inside them, the EM pass 1 line fills them with a smooth curve, EM pass 2 lies almost on top of pass 1, and the true field is shown for comparison"}
...
# Reproduce the EM fills: fit, refill the gaps with the conditional mean,
# refit, refill again (what refine = TRUE does internally).
est_p0 <- infer_kernel_params(obs_data, coordinates, nt = nt, period = period)
g_p1   <- weave:::complete_field_cond_mean(obs_data, coordinates, n, nt,
                                           period, value = "y_obs",
                                           standardise = TRUE, hp = est_p0)
est_p1 <- infer_kernel_params(obs_data, coordinates, nt = nt, period = period,
                              refine = TRUE, refine_iter = 1)
g_p2   <- weave:::complete_field_cond_mean(obs_data, coordinates, n, nt,
                                           period, value = "y_obs",
                                           standardise = TRUE, hp = est_p1)
G1 <- t(matrix(g_p1, nrow = nt, ncol = n))
G2 <- t(matrix(g_p2, nrow = nt, ncol = n))

# the site with the most missing weeks makes the fills easiest to see
site <- which.max(tapply(is.na(obs_data$y_obs), obs_data$id, sum))

lvl <- c("plug-in (start)", "EM pass 1", "EM pass 2", "true field")
plot_df <- rbind(
  data.frame(t = 1:nt, value = G[site, ],  type = lvl[1]),
  data.frame(t = 1:nt, value = G1[site, ], type = lvl[2]),
  data.frame(t = 1:nt, value = G2[site, ], type = lvl[3]),
  data.frame(t = 1:nt, value = scale(f_true_m[site, ])[, 1], type = lvl[4])
)
plot_df$type <- factor(plot_df$type, levels = lvl)

# shaded bands for this site's missing stretches
miss  <- is.na(obs_data$y_obs[as.integer(obs_data$id) == site])
runs  <- rle(miss)
ends  <- cumsum(runs$lengths)
bands <- data.frame(xmin = (ends - runs$lengths + 1)[runs$values] - 0.5,
                    xmax = ends[runs$values] + 0.5)

ggplot() +
  geom_rect(data = bands, aes(xmin = xmin, xmax = xmax,
                              ymin = -Inf, ymax = Inf),
            fill = weave_grid, alpha = 0.6) +
  geom_line(data = plot_df, aes(t, value, colour = type, linewidth = type)) +
  scale_colour_manual(values = c("plug-in (start)" = weave_pal[["sage"]],
                                 "EM pass 1"       = "#6fb5a0",
                                 "EM pass 2"       = weave_cols[["estimate"]],
                                 "true field"      = weave_cols[["truth"]]),
                      name = NULL) +
  scale_linewidth_manual(values = c(0.5, 0.7, 0.7, 0.5), guide = "none") +
  labs(x = "Week", y = "Standardised latent field",
       title = sprintf("Site %d: the EM loop fills the gaps", site),
       subtitle = "shaded = missing weeks · the passes only move the field inside them") +
  theme_weave()
```
...
badly distorting the length-scales. (The nugget and the Negative-Binomial
dispersion of part 3 are two views of the same count scatter — how they
relate, and why that is not double counting, is explained there.)
:::

Evaluating \( \log|K| \) and \( g^\top K^{-1} g \) sounds hopeless, for two
compounding reasons. First, \( K \) has one row and one column per
site-week *cell* — `r n * nt` × `r n * nt` even in this small example, and
around 260,000 × 260,000 (68 billion entries) at a realistic 1000 sites ×
five years — and determinants and inverses cost \( O(N^3) \): work that
grows with the *cube* of the cell count. Second, this is the inner loop —
the optimiser needs the score at every candidate \( \theta \) it tries,
typically hundreds of evaluations. Because the kernel is separable, both
quantities instead come exactly from eigendecompositions of the two *small*
kernels, at \( O(n^3 + n_t^3) \) cost instead of
...
The clearest check is to draw the **fitted** kernels (teal) on top of the
**true** ones (deep green, dashed). If the method worked, they overlap.
...
::: {.intuition}
**Haven't we counted the observation noise twice?** The nugget (part 2) and
the dispersion \( r \) describe the same physical fact — counts scatter
around their rate — but on different scales and with different jobs. The
nugget is that scatter as seen on the log scale: one Gaussian variance for
every cell, used while *learning* — it stops the kernels explaining noise
with smoothness, and it tells the smoother how much to de-weight each
observed week. The Negative-Binomial is the same scatter on the natural
count scale, heteroscedastic as real counts are, and it is used exactly
once: to add the spread of a future count around its rate when the interval
is assembled. There is no double counting, because the two feed different
terms of the law of total variance above: noisy observations limit how well
we can know \( \lambda \) (the nugget's contribution, inside the rate
uncertainty), while counts scatter around \( \lambda \) even if it were
known exactly (the dispersion's contribution).

The two are quantitatively linked, too. On the log scale, Negative-Binomial
noise has approximate variance \( 1/\lambda + 1/r \) — nearly constant once
counts are moderately large. That is exactly why a single homoscedastic
nugget is a reasonable stand-in during estimation, and why the approximation
frays at very low counts, where the \( 1/\lambda \) term takes over (see the
caveats below).
:::
...
The teal line is the predicted mean rate and the teal band the 95% prediction
interval. It should track the deep-green true-mean line and cover the points —
including the rose held-out weeks it never saw.
...
ggplot() +
  ...
  labs(x = "Week", y = "Cases",
       ...
       subtitle = "teal = predicted mean + 95% interval · deep green = true mean · rose = held-out truth") +
  theme_weave()
```
...
- **Plug-in, not Bayesian.** A full Bayesian treatment would average the
  score over every latent field consistent with the counts; we condition on
  a single noisy estimate instead and treat it as data. Because that
  estimate carries observation noise, some of the noise gets read as real
  short-range structure, which *attenuates* the length-scales
  (`periodic_scale` and `long_term_scale` tend to come out a little low).
  The nugget absorbs most of this — it gives the noise somewhere else to
  go — but not all of it.
- **`log(1 + y)`** is a crude stand-in for the latent log-rate. It is
  poorest at very low counts, where the `+1` dominates (the step from 0 to
  1 case is huge on this scale) and where Poisson-level noise is strongly
  asymmetric after the log — so sites with sparse counts contribute a
  distorted signal.
- **Per-site scaling** puts every site on the same footing so one global
  ...
  weeks, and a site whose true variability differs from the rest is forced
  to look the same.
- **One homoscedastic nugget.** Real count noise is heteroscedastic — a
  high-count week is noisier in absolute terms than a low-count one — but
  the model summarises all of it with the single ratio \( \eta \). The
  effect is mild over-smoothing where counts are high and under-smoothing
  where they are low.
...
- **The dispersion \( r \)** is recovered by a rough method of moments
  ...
  sensitive to outliers and to misfit in the mean. Errors in \( r \) change
  the *width* of the count prediction interval (too-small \( r \) widens
  it, too-large narrows it), not the predicted rate.
- **A point estimate.** We report the maximiser, with no uncertainty on the
  hyperparameters themselves — the prediction interval treats them as known.
  With plenty of sites and weeks this matters little (the hyperparameters
  are well pinned down); with short series or few sites the interval is
  somewhat narrower than it should be.
:::
