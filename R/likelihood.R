log_likelihood <- function(f, dist_k_inv, time_k_inv, y, mu, n, nt) {
  f_mat <- matrix(f, nrow = n, ncol = nt, byrow = TRUE)
  quad_term <- -0.5 * sum((dist_k_inv %*% f_mat) * (f_mat %*% time_k_inv))  # Optimized using element-wise multiplication

  # Distribution term
  w <- !is.na(y)

  # Poisson term using dpois
  lambda <- exp(f[w] + mu[w])  # Mean parameter for Poisson
  likelihood_term <- sum(dpois(y[w], lambda = lambda, log = TRUE))

  as.numeric(quad_term + likelihood_term)
}

log_likelihood_gradient <- function(f, dist_k_inv, time_k_inv, y, mu, n, nt) {
  f_mat <- matrix(f, nrow = n, ncol = nt, byrow = TRUE)

  # Compute the gradient of the quadratic term using matrix operations
  grad_quad_term_matrix <- -2 * dist_k_inv %*% f_mat %*% time_k_inv
  ...
  grad_likelihood_term[w] <- y[w] - exp(f[w] + mu[w])

  as.numeric(grad_quad_term + grad_likelihood_term)
}

log_likelihood_hessian <- function(f, dist_k_inv, time_k_inv, y, mu, n, nt) {
  ...
  mu <- exp(f[w] + mu[w])  # Mean parameter
