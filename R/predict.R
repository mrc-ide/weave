...
#   f = u - K S^T a.
# -----------------------------------------------------------------------------
gp_field_draw <- function(g_obs, obs_idx, N, space_mat, time_mat, noise_var,
                          kdiag, Rs_chol, Rt_chol, tol = 1e-6) {
  u <- quick_mvnorm_chol(Rs_chol, Rt_chol)
  eps <- stats::rnorm(length(obs_idx), sd = sqrt(noise_var))
  alpha <- pcg((u[obs_idx] + eps) - g_obs, obs_idx, N,
               space_mat, time_mat, noise_var, kdiag, tol = tol)
...
gp_predict <- function(obs_data, coordinates, hyperparameters, nt, period,
  ...
  if (!all(need %in% names(hp))) {
    stop("`hyperparameters` must contain: ", paste(need, collapse = ", "),
         call. = FALSE)
  }
  ...
  n <- length(ids)
  if (length(times) != nt) {
    stop("`nt` does not match the number of unique times in `obs_data`.",
         call. = FALSE)
  }
  ...
  alpha0 <- pcg(g_obs, obs_idx, N, space_mat, time_mat, noise_var, kdiag, tol = pcg_tol)
  ...
  if (n_draws >= 1) {
    ...
    if (is.null(r)) {
      lam_obs <- exp(Zmat)[cbind(match(obs_data$id[ok], ids),
