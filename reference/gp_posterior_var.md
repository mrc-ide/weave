...
is the posterior variance had \*every\* cell been observed – available
in closed form through the Kronecker eigendecomposition, no draws
needed. Missing cells change the variance only locally, so the draws are
spent purely on that correction: each zero-mean perturbation draw
